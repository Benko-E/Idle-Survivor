...
const names = (tier: number) => spellsOfTier(tier).map((d) => d.displayName)
...
check('shelved spells are in no tier', [1, 2, 3].every((t) => !spellsOfTier(t).some((d) => ['Frost Nova', 'Curse of Withering', 'Vortex', 'Entangling Roots'].includes(d.displayName))))

{
  ...
  const [first, second] = spellsOfTier(3)
  takeSpell(w, second)
  takeSpell(w, first)
